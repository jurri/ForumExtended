...
function FE_ValidVote($txt){
  ...
  while (strpos($vote_txt, '[option='.$i.']')){
    ...
    if ($i>1) for($j=1;$j<$i;$j++) if ($opt[$i] < $opt[$j]) $valid = FALSE; #Überprüft ob die Reihenfolge richtig ist 
    if (strpos($vote_txt, '[option='.$i.']',$opt[$i]+1)) $valid = FALSE; #Überprüft ob eine Option doppelt da ist
    $i++;   
  }
...
function FE_CreateVote($post_id,$txt){
  ...
  if ( db_num_rows($erg) == 1 ){
    if ($options == 0) { //Lösche den Datenbankeintrag, wenn kein Vote mehr in dem Post enthalten ist
      ...
      if ($resanz == $options) return(TRUE); //Ändere nichts, wenn sich am Vote nichts geändert hat (Anzahl der Optionen)
