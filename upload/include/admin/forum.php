...
switch ( $um ) {
  ...
  case 'newForum' :
      ...
      if ($menu->getA(2) == 'c' AND is_numeric($menu->getE(2))) {$cid = $menu->getE(2); }
		...
		$ar['m_views'] = escape($_POST['t_m_views'],'string');
		$ar['m_reply'] = escape($_POST['t_m_reply'],'string');
		$ar['m_start'] = escape($_POST['t_m_start'],'string');
		$ar['name'] = escape($_POST['name'],'string');
		$ar['text'] = escape($_POST['text'],'string');
		$ar['checkerw'] = '';
		
		if($_POST['ch_recht_erw'] == 1){
			$ar['checkerw'] = 'checked';
		}
		...
		if(isset($_POST['sub_view'])){
			$ar['m_views'] .= escape($_POST['view_erw'],'integer')."\n";
		}
		
		if(isset($_POST['sub_reply'])){			
			$ar['m_reply'] .= escape($_POST['reply_erw'],'integer')."\n";
		}
		
		if(isset($_POST['sub_start'])){		
			$ar['m_start'] .= escape($_POST['start_erw'],'integer')."\n";
		}
		
		if(isset($_POST['sub_test'])){
			//echo escape($_POST['t_m_start'],'string');
			
			$teile = explode("\n", escape($_POST['t_m_start'],'string'));
			$teilcount = count($teile) - 1;
			while($teilcount > 0){
				$teilcount--;
				$start = $teile[$teilcount];
				echo $start.'<br/>';
			}
			
			echo $_POST['ch_recht_erw'];
		}
      ...
      forum_admin_selectcats(0,'',$ar['kats'],$cid);
			...
			db_query("INSERT INTO prefix_forums (cid,view,start,reply,pos,name,besch,erwrecht) VALUES (".$cid.",".$view.",".$start.",".$reply.",".$a.",'".$name."','".$text."', $erweitert)");
			
			$forumpk = db_fetch_assoc(db_query("SELECT max(id) pk FROM prefix_forums"));
			$forumpk = $forumpk['pk'];
			
			if($erweitert == 1){
				$erweitert = 1;
				$teile = explode("\n", escape($_POST['t_m_views'],'string'));
				$teilcount = count($teile) - 1;
				while($teilcount > 0){
					$teilcount--;
					$view = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($forumpk, $view, 1)");
				}
				
				$teile = explode("\n", escape($_POST['t_m_reply'],'string'));
				$teilcount = count($teile) - 1;
				while($teilcount > 0){
					$teilcount--;
					$reply = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($forumpk, $reply, 2)");
				}
				...
				while($teilcount > 0){
					$teilcount--;
					$start = $teile[$teilcount];
					db_query("INSERT INTO prefix_forumrecht (recht_fkforum, recht_fkrecht, recht_bearbeiten) VALUES ($forumpk, $start, 3)");
				}
			}
